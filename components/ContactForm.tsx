...
const divisionQueryMap:Record<string,(typeof divisions)[number]>={digital:"Startek Digital","web-hub":"Startek Web Hub","print-hub":"Startek Print Hub"};
const fieldLimits={fullName:100,email:254,phone:32,company:120,details:2000} as const;
...
export function ContactForm(){
  ...
  useEffect(()=>{
    ...
    const mapped=division?divisionQueryMap[division]:undefined;
  ...
  function validate(){const next:Errors={};if(!form.fullName.trim())next.fullName="Please enter your full name.";else if(form.fullName.length>fieldLimits.fullName)next.fullName=`Use ${fieldLimits.fullName} characters or fewer.`;if(!form.email.trim())next.email="Please enter your email address.";else if(form.email.length>fieldLimits.email)next.email=`Use ${fieldLimits.email} characters or fewer.`;else if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))next.email="Enter a valid email address.";if(form.phone.length>fieldLimits.phone)next.phone=`Use ${fieldLimits.phone} characters or fewer.`;if(form.company.length>fieldLimits.company)next.company=`Use ${fieldLimits.company} characters or fewer.`;if(!divisions.includes(form.division))next.division="Select a valid service division.";if(!form.details.trim())next.details="Tell us a little about your project.";else if(form.details.length>fieldLimits.details)next.details=`Use ${fieldLimits.details} characters or fewer.`;if(!["Email","Phone","WhatsApp"].includes(form.contactMethod))next.contactMethod="Select a preferred contact method.";setErrors(next);return Object.keys(next).length===0}
  ...
  return <form className="contact-form" onSubmit={submit} noValidate>
    <div className="form-grid">
      <Field id="fullName" label="Full Name" required error={errors.fullName}><input id="fullName" name="fullName" autoComplete="name" maxLength={fieldLimits.fullName} value={form.fullName} onChange={e=>update("fullName",e.target.value)} aria-invalid={Boolean(errors.fullName)} aria-describedby={errors.fullName?"fullName-error":undefined}/></Field>
      <Field id="email" label="Email Address" required error={errors.email}><input id="email" name="email" type="email" autoComplete="email" maxLength={fieldLimits.email} value={form.email} onChange={e=>update("email",e.target.value)} aria-invalid={Boolean(errors.email)} aria-describedby={errors.email?"email-error":undefined}/></Field>
      <Field id="phone" label="Phone Number" error={errors.phone}><input id="phone" name="phone" type="tel" autoComplete="tel" maxLength={fieldLimits.phone} value={form.phone} onChange={e=>update("phone",e.target.value)} aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone?"phone-error":undefined}/></Field>
      <Field id="company" label="Company" error={errors.company}><input id="company" name="company" autoComplete="organization" maxLength={fieldLimits.company} value={form.company} onChange={e=>update("company",e.target.value)} aria-invalid={Boolean(errors.company)} aria-describedby={errors.company?"company-error":undefined}/></Field>
