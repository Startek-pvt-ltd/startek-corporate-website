...
export function ContactForm(){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!validate()){setStatus("error");return}setStatus("submitting");await new Promise(resolve=>setTimeout(resolve,400));setStatus("not-connected")}
  ...
  return <form className="contact-form" onSubmit={submit} noValidate>
    ...
    <div className={`form-status ${status}`} aria-live="polite">{status==="not-connected"&&<p><strong>Online submission is being finalized.</strong> No enquiry was sent. Please use Startek&apos;s official social channels to contact us in the meantime.</p>}{status==="error"&&Object.keys(errors).length>0&&<p>Please correct the highlighted fields before continuing.</p>}</div>
  </form>
}
