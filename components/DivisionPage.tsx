...
export function DivisionPage({data}:{data:DivisionPageData}){const contactHref=`/contact?division=${data.theme==="web"?"web-hub":data.theme==="print"?"print-hub":"digital"}`;return <main className={`division-page theme-${data.theme}`}><Navbar/>
  <section className="division-hero" id="home"><div className="division-hero-grid" aria-hidden="true"/><div className="division-hero-signal" aria-hidden="true"><i/><i/><i/><span/></div><div className="container division-hero-inner"><Reveal><p className="eyebrow"><span>+</span>{data.eyebrow}</p><h1>{data.headline.split("\n").map((line,i)=><span key={line}>{line}{i<data.headline.split("\n").length-1&&<br/>}</span>)}</h1><p>{data.heroText}</p><div className="hero-actions"><Link className="button" href={contactHref}>{data.primaryCta}<ArrowUpRight size={16}/></Link><Link className="text-link" href="#work">{data.secondaryCta}<ArrowDown size={15}/></Link></div></Reveal></div><div className="division-hero-label" aria-hidden="true"><span>{data.theme.toUpperCase()}</span><i/></div></section>
  ...
  <section className="section division-work" id="work"><div className="container"><Reveal><div className="heading-row"><SectionHeading eyebrow="Featured Work" title={data.projectsTitle} copy={data.projectsText}/><Link className="text-link" href={contactHref}>{data.projectsCta}<ArrowUpRight size={16}/></Link></div></Reveal><div className="division-project-grid">{data.projects.map((project,i)=><Reveal key={project.name} delay={i*.06}><ProjectCard project={project} index={i}/></Reveal>)}</div></div></section>
  ...
  <section className="section division-testimonials"><div className="container"><Reveal><SectionHeading eyebrow="Client Perspectives" title="What Our Clients Say" copy="Sample content shown to demonstrate the layout. Approved client feedback will replace these placeholders."/></Reveal><div className="testimonial-track">{data.testimonials.map((testimonial,i)=><Reveal key={`${testimonial.company}-${i}`} delay={i*.06}><TestimonialCard testimonial={testimonial}/></Reveal>)}</div></div></section>
