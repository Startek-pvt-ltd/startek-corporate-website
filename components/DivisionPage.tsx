...
import type { DivisionPageData,ServiceItem } from "@/data/division-pages";
...
const digitalServiceGroups=["Strategy","Content & Creative","Performance"] as const;

function ServiceGrid({services,isPrint=false}:{services:ServiceItem[];isPrint?:boolean}){return <div className={`division-service-grid ${isPrint?"category-grid":""}`}>{services.map((service,i)=>{const Icon=iconMap[service.icon]||Sparkles;return <Reveal key={service.title} delay={(i%4)*.04}><article className="division-service-card"><div className="service-card-top"><Icon size={21}/><span>{String(i+1).padStart(2,"0")}</span></div><h3>{service.title}</h3><p>{service.description}</p>{service.items&&<div className="category-items">{service.items.map(item=><span key={item}>{item}</span>)}</div>}</article></Reveal>})}</div>}
...
export function DivisionPage({data}:{data:DivisionPageData}){const brandKey=(data.theme==="web"?"web-hub":data.theme==="print"?"print-hub":"digital") satisfies Exclude<BrandKey,"startek">;const contactHref=`/contact?division=${brandKey}`;const brand=brandAssets[brandKey];const socials=divisionSocialLinks[brandKey];return <main className={`division-page theme-${data.theme}`}><Navbar/>
  <section className="division-hero" id="home"><div className="division-hero-grid" aria-hidden="true"/><div className="division-hero-signal" aria-hidden="true"><i/><i/><i/><span/></div><Link className="division-brand-visual" href="#services" aria-label={`Explore ${brand.name} services`}><span className="division-brand-orbit" aria-hidden="true"><i/><i/><i/></span><Image className="division-brand-logo" src={brand.logo} alt={`${brand.name} official logo`} width={640} height={640} sizes="(max-width: 820px) 180px, 320px"/><span className="division-brand-hint">Explore services <ArrowDown size={13} aria-hidden="true"/></span></Link><div className="container division-hero-inner"><Reveal><p className="eyebrow"><span>+</span>{data.eyebrow}</p><h1>{data.headline.split("\n").map((line,i)=><span key={line}>{line}{i<data.headline.split("\n").length-1&&<br/>}</span>)}</h1><p>{data.heroText}</p><div className="hero-actions"><Link className="button" href={contactHref}>{data.primaryCta}<ArrowUpRight size={16} aria-hidden="true"/></Link><Link className="text-link" href="#work">{data.secondaryCta}<ArrowDown size={15} aria-hidden="true"/></Link></div></Reveal></div><div className="division-hero-label" aria-hidden="true"><span>{data.theme.toUpperCase()}</span><i/></div></section>
  ...
  <section className="section division-services" id="services"><div className="container"><Reveal><SectionHeading eyebrow={`${data.brand} Services`} title={data.servicesTitle} copy={data.theme==="print"?"A coordinated range of print and physical brand solutions, selected around the application, quantity and specification.":"Specialist services that can work independently or combine into one focused solution."}/></Reveal>{data.theme==="digital"?<div className="digital-service-groups">{digitalServiceGroups.map(group=><div className="digital-service-group" key={group}><p className="service-group-label">{group}</p><ServiceGrid services={data.services.filter(service=>service.serviceGroup===group)}/></div>)}</div>:<ServiceGrid services={data.services} isPrint={data.theme==="print"}/>}</div></section>
